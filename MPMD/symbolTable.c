...
SPtr initializeSymbolTable(){
	...
	FPtr fnPtr = (FPtr)malloc(MAX_SIZE*sizeof(struct functions1));
	SPtr symTabPtr = (SPtr)malloc(sizeof(struct symbolTable));
	symTabPtr->fp= fnPtr;
	...
	for(a=0;a<MAX_SIZE;a++)
	{
		...
		varPtr = (VPtr)malloc(MAX_SIZE*sizeof(struct variables1));
     	initializeVPtr(fnPtr,varPtr,a);
		for(b=0;b<MAX_SIZE;b++)
		{

			RPtr recPtr;
			recPtr = (RPtr)malloc(MAX_SIZE*sizeof(struct records1));
...
{	int flag;
	...
	int req=0;
	//temp = ast;
	//checkReturnValues(ast,symPtr,fglobal,&req,fname);
	//printf("dfdfdf\n");
	//temp = ast;
	setIfAssigned(ast,symPtr,fglobal);
	//printf("11\n");
	//temp = ast;
